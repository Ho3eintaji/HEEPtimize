...
void runCycles(unsigned int ncycles, Vtb_system *dut, Monitor *mon, uint8_t gen_waves, VerilatedFstC *trace);
...
int main(int argc, char *argv[])
{
    ...
    case BOOT_MODE_FORCE:
        ...
        runCycles(1, dut, NULL, gen_waves, trace);
    ...
    while (!cntx->gotFinish() && cntx->time() < (max_cycles << 1) && dut->exit_valid_o == 0) {
        ...
        runCycles(RUN_CYCLES, dut, mon, gen_waves, trace);
    }
    if (cntx->time() >= (max_cycles << 1)) {
        TB_WARN("Max simulation cycles reached");
    }
    mon->carusExecTimeEn(false);
    ...
    if (dut->exit_valid_o) {
        ...
        runCycles(10, dut, mon, gen_waves, trace);
    } else {
...
void rstDut(Vtb_system *dut, uint8_t gen_waves, VerilatedFstC *trace) {
    ...
    runCycles(PRE_RESET_CYCLES, dut, NULL, gen_waves, trace);
    dut->rst_ni = 0;
    TB_LOG(LOG_MEDIUM, "- reset asserted");
    runCycles(RESET_CYCLES, dut, NULL, gen_waves, trace);
    TB_LOG(LOG_MEDIUM, "- reset released");
    dut->rst_ni = 1;
    runCycles(POST_RESET_CYCLES, dut, NULL, gen_waves, trace);
}
