...
// Launch cgra matmul
void cgraMatMulLaunch(uint32_t *A, uint32_t *B, uint32_t *C, uint32_t A_rows, uint32_t A_cols, uint32_t B_cols);
...
int main(void)
{
    ...
    t1 = timer_get_cycles();
    // launch cgra matmul
    cgraMatMulLaunch(A_ram, B_ram, R_cgra, A_ROWS, A_COLS, B_COLS);
