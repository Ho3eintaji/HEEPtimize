...
// The input "bits" represents the number of bits used to represent the indices of the data array
// BIT_REVERSE
void fft_permutate(fft_complex_t data[], unsigned bits) {
  unsigned size  = 1 << bits; // total size of the data array
  unsigned shift = 32 - bits; // number of bits to shift when working with binary reversed indices
  for(unsigned i = 1; i < size - 1; i++) {
    unsigned z = rbit(i) >> shift;
    if(z > i) {
      fft_t //swap the real (r) and imaginary (i) parts of the complex numbers at positions i and z
...
#ifdef FFT_DIT  // with decimate in time permutation is before the FFT
  fft_permutate(complex, bits);
#endif
  fft_forward(complex, bits);
#ifdef FFT_DIF  // with decimate in frequency permutation is after the FFT
  fft_permutate(complex, bits);
#endif
}
