...
#include <stdio.h>
#include "transformerBlock_nmc.h"
#include "timer_sdk.h"
                       ...
                       quant_bit_width *intermediate, quant_bit_width *qkv, quant_bit_width* ram_buffer)
{
    ...
    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->addNorm.weight_), ram_buffer, 800) != FLASH_OK)return -1;
    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->addNorm.bias_), ram_buffer+400, 800) != FLASH_OK)return -1;
    ...
    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->patchEmbedding->weight), ram_buffer, 6400*2) != FLASH_OK)return -1;
    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->patchEmbedding->bias), ram_buffer+6400, 32) != FLASH_OK)return -1;
    transformerBlock->patchEmbedding->weight = ram_buffer;
    transformerBlock->patchEmbedding->bias = ram_buffer+6400;
    ...
    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->addNorm2.weight_), ram_buffer, 32) != FLASH_OK)return -1;
    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->addNorm2.bias_), ram_buffer+16, 32) != FLASH_OK)return -1;
    transformerBlock->addNorm2.weight_ = ram_buffer;
    transformerBlock->addNorm2.bias_ = ram_buffer+16;
    normalize(&transformerBlock->addNorm2, output, output); // consider doing this with carus kernel
    ...
    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->token->pos_matrix_), ram_buffer, 1936*2) != FLASH_OK)return -1;
    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->token->cls_token_vector_), ram_buffer+1936, 32) != FLASH_OK)return -1;
    transformerBlock->token->pos_matrix_ = ram_buffer;
    transformerBlock->token->cls_token_vector_ = ram_buffer+1936;
    ...
    for (int l = 0; l < 4; l++) // loop through the 4 transformer layers
    {
        ...
        if(l!=3){ // load successive layer
            if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->transformer_layer_0_addNorm[l+1].weight_), (uint32_t *)transformerBlock->transformer_layer_0_addNorm[0].weight_, 32) != FLASH_OK)return 1;
            if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->transformer_layer_0_addNorm[l+1].bias_), (uint32_t *)transformerBlock->transformer_layer_0_addNorm[0].bias_, 32) != FLASH_OK)return 1;
        }
        #ifdef PRINT_INTERMEDIATE_CYCLES
            normalize_time = timer_stop();
            PRINTF("normalize time[%d]: %d\n", l, normalize_time);
        #endif
        #ifdef USE_DOUBLE_HEAD_SELF_ATTN
            for (int n = 0; n < NUM_HEAD; n+=2)   // loop through the 4 heads in the multi-head attention mechanism
            {
                #ifdef PRINT_INTERMEDIATE_CYCLES
                    timer_start();
                #endif
                compute_DoubleHeadSelfAttn(transformerBlock->selfatten[ n],transformerBlock->selfatten[ n + 1], input_normalized,
                                        output + n * (seq_len * transformerBlock->head_hidden_size_), output + (n+1) * (seq_len * transformerBlock->head_hidden_size_), qkv, ram_buffer, intermediate);

                #ifdef PRINT_INTERMEDIATE_CYCLES
                    int selfattn_time = timer_stop();
                    PRINTF("double head time[%d][%d]: %d\n", l, n/2, selfattn_time);
                #endif
                if(l!=3){ // load successive layer from flash
                    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->selfatten[(l+1)*NUM_HEAD + n ]->query_layer->weight),(uint32_t *)transformerBlock->selfatten[n]->query_layer->weight, 128) != FLASH_OK)return 1;
                    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->selfatten[(l+1)*NUM_HEAD + n + 1]->query_layer->weight),(uint32_t *)transformerBlock->selfatten[n+1]->query_layer->weight, 128) != FLASH_OK)return 1;
                    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->selfatten[(l+1)*NUM_HEAD + n ]->key_layer->weight),(uint32_t *)transformerBlock->selfatten[n]->key_layer->weight, 128) != FLASH_OK)return 1;
                    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->selfatten[(l+1)*NUM_HEAD + n + 1]->key_layer->weight),(uint32_t *)transformerBlock->selfatten[n+1]->key_layer->weight, 128) != FLASH_OK)return 1;
                    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->selfatten[(l+1)*NUM_HEAD + n ]->value_layer->weight),(uint32_t *)transformerBlock->selfatten[n]->value_layer->weight, 128) != FLASH_OK)return 1;
                    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->selfatten[(l+1)*NUM_HEAD + n + 1]->value_layer->weight),(uint32_t *)transformerBlock->selfatten[n+1]->value_layer->weight, 128) != FLASH_OK)return 1;
                }
            }
        #else
            ...
            timer_start();
        #endif
        multihead_transpose(output, intermediate, seq_len, transformerBlock->head_hidden_size_, transformerBlock->num_heads_); // TODO : do with DMA (while reading from flash)
        #ifdef PRINT_INTERMEDIATE_CYCLES
        ...
        if(l!=3){ // load successive layer from flash
            if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->condense[l+1]->weight),(uint32_t *)transformerBlock->condense[0]->weight, 512) != FLASH_OK)return 1;
            if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->condense[l+1]->bias),(uint32_t *)transformerBlock->condense[0]->bias, 32) != FLASH_OK)return 1;
        }
        #ifdef PRINT_INTERMEDIATE_CYCLES
        ...
        if(l!=3){ // load successive layer
            if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->transformer_layer_1_addNorm[l+1].weight_), (uint32_t *)transformerBlock->transformer_layer_1_addNorm[0].weight_, 32) != FLASH_OK)return 1;
            if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->transformer_layer_1_addNorm[l+1].bias_), (uint32_t *)transformerBlock->transformer_layer_1_addNorm[0].bias_, 32) != FLASH_OK)return 1;
        }
        #ifdef PRINT_INTERMEDIATE_CYCLES
            normalize_time = timer_stop();
            PRINTF("normalize time[%d]: %d\n", l, normalize_time);
        ...
        if(l!=3){ // load successive layer from flash
            if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->feedForward0[l+1]->weight),(uint32_t *)transformerBlock->feedForward0[0]->weight, 128) != FLASH_OK)return 1;
            if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->feedForward0[l+1]->bias),(uint32_t *)transformerBlock->feedForward0[0]->bias, 8) != FLASH_OK)return 1;
        }
        #ifdef PRINT_INTERMEDIATE_CYCLES
        ...
        if(l!=3){ // load successive layer from flash
            if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->feedForward1[l+1]->weight),(uint32_t *)transformerBlock->feedForward1[0]->weight, 128) != FLASH_OK)return 1;
            if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->feedForward1[l+1]->bias),(uint32_t *)transformerBlock->feedForward1[0]->bias, 32) != FLASH_OK)return 1;
        }
        #ifdef PRINT_INTERMEDIATE_CYCLES
    ...
    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->mlp_head_norm.weight_), ram_buffer, 32) != FLASH_OK)return -1;
    if(w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->mlp_head_norm.bias_), ram_buffer+16, 32) != FLASH_OK)return -1;  
    ...
    if (w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->mlp_head_linear->weight), ram_buffer, 512) != FLASH_OK)return -1;
    if(w25q128jw_read((uint32_t)heep_get_flash_address_offset((uint32_t *)transformerBlock->mlp_head_linear->bias), ram_buffer+256, 32) != FLASH_OK)return -1;  
    transformerBlock->mlp_head_linear->weight = ram_buffer;
    transformerBlock->mlp_head_linear->bias = ram_buffer+256;
