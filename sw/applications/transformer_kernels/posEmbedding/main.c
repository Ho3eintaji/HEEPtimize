...
quant_bit_width __attribute__((section(".xheep_data_interleaved"))) pos_matrix[INPUT_SIZE] = {0};
quant_bit_width __attribute__((section(".xheep_data_interleaved"))) cls_token_vector[INPUT_DIM] = {0};

int main() {
    // Initialize input, pos_matrix, and cls_token_vector with some values
    for (int i = 0; i < INPUT_SIZE; i++) {
        input[i] = i; // Example initialization
        pos_matrix[i] = i; // Example initialization
    }
