                ...
                const int32_t input_len, const int32_t input_depth, const int32_t output_len)
{
    ...
    // loop 0 - unrolled
    register signed char *filter_address = filter;
    for (int32_t w_n = 0; w_n < 128; w_n++)
    {
        sum = 0;
        for (int32_t w_j = 0; w_j < input_depth; w_j++)
        {
            register int16_t *data_address = &data[input_len * w_j]; // reset data address every time we go to a deeper layer
            filter_address++;
    ...
    for (int32_t start_index = 1; start_index < input_len - 1; start_index++)
    {
        ...
        for (int32_t w_n = 0; w_n < 128; w_n++)
        {
            ...
            for (int32_t w_j = 0; w_j < input_depth; w_j++)
            {
                register int16_t *data_address = &data[input_len * w_j + offset]; // reset data address every time we go to a deeper layer
    ...
    for (int32_t w_n = 0; w_n < 128; w_n++)
    {
        ...
        for (int32_t w_j = 0; w_j < input_depth; w_j++)
        {
            register int16_t *data_address = &data[input_len * w_j + input_len - 2]; // reset data address every time we go to a deeper layer
