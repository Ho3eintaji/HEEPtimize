...
#ifndef _CONV_CGRA_WP_H_
#define _CONV_CGRA_WP_H_
...
void config_wp(int input_channel, int output_channel)
{
    ...
    for(int16_t i = 0; i < 3; i++) {
        cgra_input_wp[0][i] =   &(input[0][input_channel][0][i]);
        cgra_input_wp[1][i] =   &(input[0][input_channel][1][i]);
        cgra_input_wp[2][i] =   &(input[0][input_channel][2][i]);

    }

    cgra_input_wp[3][0]= &CGRA_output[output_channel];

    for(int16_t i = 3; i < 6; i++){
        cgra_input_wp[0][i] = filter[output_channel][input_channel][0][i-3];
        cgra_input_wp[1][i] = filter[output_channel][input_channel][1][i-3];
        cgra_input_wp[2][i] = filter[output_channel][input_channel][2][i-3];
    }

#elif TRANSPOSE == 0
    for(int16_t i = 0; i < 3; i++) {
        cgra_input_wp[0][i] =   &(input[0][input_channel][i][0]);
        cgra_input_wp[1][i] =   &(input[0][input_channel][i][1]);
        cgra_input_wp[3][i] =   &(input[0][input_channel][i][2]);
        cgra_input_wp[2][i] = filter[output_channel][input_channel][i][2];
    }

    cgra_input_wp[3][3]= &output_from_CGRA[output_ch];

    for(int16_t i = 3; i < 6; i++){
        cgra_input_wp[0][i] = filter[output_channel][input_channel][i-3][0];
        cgra_input_wp[1][i] = filter[output_channel][input_channel][i-3][1];
