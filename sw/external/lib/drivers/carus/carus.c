...
int carus_get_ctl(const uint8_t inst, carus_ctl_t *ctl) {
    ...
    if (carus_set_mode(inst, CARUS_MODE_CFG) != 0) return -1;
    ...
    if (carus_set_mode(inst, CARUS_MODE_MEM) != 0) return -1;
...
int carus_init(const uint8_t inst) {
    ...
    if (carus_set_ctl(inst, &ctl) != 0) return -1;
    ...
    if (carus_set_cfg(inst, &cfg) != 0) return -1;
...
int carus_load_kernel(const uint8_t inst, const uint32_t *kernel, const uint32_t size, const uint32_t offs) {
    ...
    // Check kernel size
    if (size > (CARUS_EMEM_SIZE - CARUS_PARAMS_SIZE - 1)) return -1;
...
int carus_exec_kernel(const uint8_t inst, const uint32_t *kernel, const uint32_t size) {
    // Load the kernel
    if (carus_load_kernel(inst, kernel, size, NULL) != 0) return -1;
    
    // Start the kernel and wait completion
    if (carus_run_kernel(inst) != 0) return -1;
    if (carus_wait_done(inst) != 0) return -1;
