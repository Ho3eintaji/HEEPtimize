...
module heepatia_bus #(
    ...
    input  obi_pkg::obi_req_t  [heepatia_pkg::DMAMasterPortsNum-1:0] heep_dma_read_req_i,
    output obi_pkg::obi_resp_t [heepatia_pkg::DMAMasterPortsNum-1:0] heep_dma_read_resp_o,

    input  obi_pkg::obi_req_t  [heepatia_pkg::DMAMasterPortsNum-1:0] heep_dma_write_req_i,
    output obi_pkg::obi_resp_t [heepatia_pkg::DMAMasterPortsNum-1:0] heep_dma_write_resp_o,

    input  obi_pkg::obi_req_t  [heepatia_pkg::DMAMasterPortsNum-1:0] heep_dma_addr_req_i,
    output obi_pkg::obi_resp_t [heepatia_pkg::DMAMasterPortsNum-1:0] heep_dma_addr_resp_o,
    ...
    output reg_pkg::reg_req_t heepatia_im2col_req_o,
    input  reg_pkg::reg_rsp_t heepatia_im2col_resp_i
  ...
  */
  generate
    for (genvar i = 0; i < CarusNumRnd; i++) begin : gen_carus_req
      assign carus_req_o[i].req   = ext_slave_req[i+1].req;
      assign carus_req_o[i].we    = ext_slave_req[i+1].we;
      assign carus_req_o[i].be    = ext_slave_req[i+1].be;
      assign carus_req_o[i].wdata = ext_slave_req[i+1].wdata;
      assign carus_req_o[i].addr  = ext_slave_req[i+1].addr - ExtSlaveAddrRules[i].start_addr;
      assign ext_slave_rsp[i+1]   = carus_resp_i[i];
    end

    for (genvar i = 0; i < CaesarNumRnd; i++) begin : gen_caesar_req
      assign caesar_req_o[i].req             = ext_slave_req[CarusNum+i+1].req;
      assign caesar_req_o[i].we              = ext_slave_req[CarusNum+i+1].we;
      assign caesar_req_o[i].be              = ext_slave_req[CarusNum+i+1].be;
      assign caesar_req_o[i].wdata           = ext_slave_req[CarusNum+i+1].wdata;
      assign caesar_req_o[i].addr            = ext_slave_req[CarusNum+i+1].addr - ExtSlaveAddrRules[CarusNum+i].start_addr; //TODO: CHECK
  ...
  assign heepatia_im2col_req_o             = ext_periph_req[HeepatiaIm2ColIdx];
  assign ext_periph_rsp[HeepatiaIm2ColIdx] = heepatia_im2col_resp_i;
